//! MCP tool implementations for Anka.

use std::sync::Mutex;

use anka_core::{Collection, Id, Rating};
...
fn missing_deck_message(name: &str, decks: &[anka_core::Deck]) -> String {
    if decks.is_empty() {
...
fn stats_weak(state: &Mutex<Collection>, args: &Value) -> ToolResult {
    ...
    let hard: Vec<Value> = due
        ...
        .take(limit as usize)
        .map(|item| {
            let (front, _back) = anka_core::front_back(&item.note.fields);
            json!({
                "cardId": item.card.id.to_string(),
                "front": front,
                "lapses": item.card.state.lapses,
...
fn fsrs_optimize(state: &Mutex<Collection>, args: &Value) -> ToolResult {
    ...
    Ok(text_content(json!({
        ...
        "applied": apply,
        "minRecommendedReviews": anka_core::MIN_REVIEWS_FOR_OPTIMIZE
    })))
}
