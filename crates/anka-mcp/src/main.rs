//! Anka MCP: local stdio against a collection, or stdio→HTTP bridge to anka-server.
...
use anka_mcp::{handle_message, ClientMessage};

#[derive(Parser, Debug)]
#[command(name = "anka-mcp", version, about = "MCP server for Anka")]
struct Args {
    /// Local collection (stdio mode)
    #[arg(long, env = "ANKA_COLLECTION")]
    collection: Option<PathBuf>,

    /// Remote MCP endpoint, e.g. http://nas:8787/mcp
    #[arg(long, env = "ANKA_MCP_URL")]
...
fn main() -> Result<()> {
    ...
    let path = args
        .collection
        .unwrap_or_else(|| PathBuf::from("collection.akdb"));
