//! Self-hosted Engram server.
//!
//! - REST API for UI (`/api/*`)
//! - Remote MCP (JSON-RPC over HTTP) at `POST /mcp`
//! - Static UI + media
//!
//! Auth: `Authorization: Bearer <ENGRAM_SERVER_TOKEN>` on `/api/*` and `/mcp`
...
#[derive(Parser, Debug)]
#[command(name = "engram-server", about = "Self-hosted Engram (API + MCP + UI)")]
struct Args {
    #[arg(long, env = "ENGRAM_COLLECTION", default_value = "/data/collection.egdb")]
    collection: PathBuf,

    #[arg(long, env = "ENGRAM_SERVER_LISTEN", default_value = "0.0.0.0:8787")]
    listen: SocketAddr,
    ...
    #[arg(long, env = "ENGRAM_SERVER_STATIC")]
...
async fn main() -> Result<()> {
    ...
    if token.is_empty() {
        tracing::warn!("ENGRAM_SERVER_TOKEN empty — auth disabled (dev only)");
    }
    ...
    // Protected API + MCP
    let protected = engram_serve::router(state.clone())
    ...
    let media_dir = engram_core::media_dir_for_collection(&args.collection);
    if media_dir.is_dir() {
...
async fn health() -> axum::Json<serde_json::Value> {
    axum::Json(serde_json::json!({ "ok": true, "app": "engram-server" }))
}
