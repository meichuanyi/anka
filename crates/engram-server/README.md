...
**Remote self-hosted Engram**: one process, one collection, one token.
...
- Local `engram-sync` multi-master is optional / backup, not the main path

## Run (local binary)

```bash
export ENGRAM_SERVER_TOKEN=please-change-me
export ENGRAM_COLLECTION=./data/collection.egdb
export ENGRAM_SERVER_STATIC=apps/desktop/dist
cargo run -p engram-server
...
Volume: `engram-data` → `/data/collection.egdb` + `/data/media/`.
...
```json
{
  "mcpServers": {
    "engram": {
      "command": "engram-mcp-http-bridge",
