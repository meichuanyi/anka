...
impl Collection {
    ...
    pub fn put_anki_id(&mut self, kind: &str, anki_id: &str, anka_id: Id) -> Result<()> {
        self.store().put_anki_id(kind, anki_id, anka_id)
    }

    pub fn anka_id_for_anki(&self, kind: &str, anki_id: &str) -> Result<Option<Id>> {
