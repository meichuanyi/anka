...
impl<'c> Store<'c> {
    ...
    pub fn put_anki_id(&self, kind: &str, anki_id: &str, anka_id: Id) -> Result<()> {
        self.conn.execute(
            "INSERT INTO anki_id_map (kind, anki_id, anka_id) VALUES (?1,?2,?3)
             ON CONFLICT(kind, anki_id) DO UPDATE SET anka_id = excluded.anka_id",
            params![kind, anki_id, anka_id.to_string()],
        )?;
        Ok(())
    }
    ...
    pub fn get_anki_id(&self, kind: &str, anki_id: &str) -> Result<Option<Id>> {
        self.conn
            .query_row(
                "SELECT anka_id FROM anki_id_map WHERE kind = ?1 AND anki_id = ?2",
                params![kind, anki_id],
                |r| r.get::<_, String>(0),
            )
            .optional()?
