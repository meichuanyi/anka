...
pub fn import_apkg(path: impl AsRef<Path>, col: &mut Collection) -> anyhow::Result<ImportReport> {
    ...
    // Map anki deck id -> engram deck id
...
/// Import Anki `revlog` rows into Engram revlog.
///
...
fn import_revlog(
    ...
    for row in rows {
        ...
        let Some(rating) = engram_core::Rating::from_u8(ease.clamp(1, 4) as u8) else {
            skipped_ease += 1;
            continue;
        };
        if ease == 0 {
        ...
        let Some(card_id) = col.engram_id_for_anki("card", &anki_card)? else {
            skipped_unmapped += 1;
            continue;
        };
        ...
        let entry = engram_core::RevlogEntry {
            id: engram_core::Id::new(),
            card_id,
            rating,
            reviewed_at,
