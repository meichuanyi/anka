//! Export an Anka collection to an Anki `.apkg` (Legacy1 / V11).
//!
...
pub fn export_apkg(
    ...
    for (anka_id, name) in &data.decks {
        if name == "Default" {
            deck_anki.insert(anka_id.clone(), DEFAULT_DECK_ID);
            continue;
        }
        let anki_id = anki_ids
            .get(&("deck".to_string(), anka_id.clone()))
        ...
        deck_anki.insert(anka_id.clone(), anki_id);
        deck_json.insert(
            anki_id.to_string(),
            json!({
                "id": anki_id,
...
/// Reverse of import's `put_anki_id`: (kind, anka_id) -> anki_id.
fn load_anki_id_map(col: &Collection) -> anyhow::Result<HashMap<(String, String), String>> {
    ...
    let mut stmt = src.prepare("SELECT kind, anki_id, anka_id FROM anki_id_map")?;
    let rows = stmt.query_map([], |r| {
        Ok((
            r.get::<_, String>(0)?,
            r.get::<_, String>(1)?,
    ...
    for row in rows {
        let (kind, anki_id, anka_id) = row?;
        out.insert((kind, anka_id), anki_id);
    }
    Ok(out)
}
