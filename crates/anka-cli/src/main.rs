...
use anka_core::{Collection, Rating};
use serde_json::json;

mod sync;

#[derive(Parser, Debug)]
#[command(name = "anka", version, about = "Modern AI-native spaced repetition")]
struct Cli {
    /// Path to the collection database (default: ./collection.akdb)
    #[arg(long, global = true, env = "ANKA_COLLECTION")]
...
enum Commands {
    ...
    /// Push/pull with anka-sync server
    Sync {
        #[command(subcommand)]
        cmd: SyncCmd,
    },
...
enum SyncCmd {
    ...
    Push {
        #[arg(long, env = "ANKA_SYNC_SERVER", default_value = "http://127.0.0.1:8788")]
        server: String,
...
fn resolve_collection(path: Option<PathBuf>) -> PathBuf {
    path.unwrap_or_else(|| PathBuf::from("collection.akdb"))
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Import { path: apkg, deck } => {
            ...
            let report = anka_apkg::import_apkg(&apkg, &mut col)?;
        ...
        Commands::Export { out, deck } => {
            ...
            let options = anka_apkg::ExportOptions { deck };
            let report = anka_apkg::export_apkg(&col, &out, options)?;
        ...
        Commands::Decks => {
            ...
            if counts.is_empty() {
                println!("no decks yet — try: anka import some.apkg");
                return Ok(());
            }
        ...
        Commands::Notes { query, limit } => {
            ...
            for n in notes {
                let (front, back) = anka_core::front_back(&n.fields);
                println!(
                    "- [{}] {} | {}",
                    &n.id.to_string()[..8],
                    truncate(&front, 60),
        ...
        Commands::Review { limit, deck, show, json } => {
            ...
            if json {
                let cards: Vec<_> = due
                    .iter()
                    .map(|item| {
                        let (front, back) = anka_core::front_back(&item.note.fields);
                        json!({
            ...
            if !show && !std::io::stdin().is_terminal() {
                bail!(
                    "review needs an interactive terminal; use --show or --json for scripts\n\
                     example: anka review --deck <name> --json"
                );
            }
            println!("{} card(s) due\n", due.len());
            if show {
            ...
            match col.optimize_fsrs(apply) {
                ...
                Err(e) => {
                    ...
                    if report_min_hint(&e) {
                        println!("hint: need more review history (~{}+ answers)", anka_core::MIN_REVIEWS_FOR_OPTIMIZE);
                    }
                    std::process::exit(1);
                }
            }
...
fn report_min_hint(e: &anka_core::Error) -> bool {
    matches!(e, anka_core::Error::Invalid(_) | anka_core::Error::Scheduler(_))
}

/// Resolve an exact deck name, listing candidates on miss (helps Chinese / hierarchical names).
fn resolve_deck_id(col: &Collection, name: &str) -> Result<anka_core::Id> {
    ...
    if decks.is_empty() {
        msg.push_str("\nno decks in collection yet — try: anka import some.apkg");
    } else {
        msg.push_str("\navailable decks:");
...
fn run_review_session(col: &mut Collection, due: Vec<anka_core::DueCard>) -> Result<()> {
